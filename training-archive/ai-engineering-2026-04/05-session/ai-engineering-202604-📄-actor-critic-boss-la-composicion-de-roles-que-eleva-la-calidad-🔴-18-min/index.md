---
title: "📄 Actor-Critic-Boss: la composición de roles que eleva la calidad 🔴 — 18 min"
source_url: "https://training.lidr.co/posts/ai-engineering-202604-📄-actor-critic-boss-la-composicion-de-roles-que-eleva-la-calidad-🔴-18-min"
archived_at: "2026-06-12T09:23:42.480Z"
group: "05-session"
---
...
![006-actor-critic-boss.jpg](./assets/006-actor-critic-boss.jpg)

**Actor.**Genera la estimación inicial a partir de la transcripción, los adjuntos y elproject_metadata. Es la llamada al LLM que elestimatorya hace hoy. No cambia. Sigue exactamente el patrón que conoces: template Jinja2 según tier, schema Pydantic en la salida, contexto CAG en el system prompt. La diferencia es que su salida deja de ser la respuesta final y pasa a ser un*candidato*.

**Critic.**Recibe el output del actor y lo evalúa contra un conjunto explícito de criterios: ¿está completo? ¿la aritmética interna cuadra? ¿los riesgos identificados son coherentes con el alcance? ¿hay contradicciones con elproject_metadata? ¿faltan componentes que la transcripción menciona explícitamente? El crítico**no genera una nueva estimación**: produce feedback estructurado sobre la estimación que recibió.
...
![image.png](./assets/5ded8b7619879a98.png)
...
**Evaluator-Optimizer:**un LLM genera, otro evalúa, se itera. Es el origen estructural delactor + critic.

- 

**Orchestrator-Workers:**un LLM central descompone tareas, delega a workers, y sintetiza resultados. Es el origen delboss.
...
![007-dos-vs-tres-roles.jpg](./assets/007-dos-vs-tres-roles.jpg)
...
**Existen criterios de evaluación claros.**El crítico necesita instrucciones específicas para evaluar. Si los criterios son "que esté bien", el patrón degenera porque el crítico no tiene material concreto sobre el que producir feedback. Para elestimator, los criterios son claros: aritmética interna, completitud de componentes, coherencia con la transcripción.
...
La regla operativa que suele funcionar:**aplica el patrón solo a los caminos críticos del producto, no a todas las llamadas al LLM**. Para elestimator, esto se traduce en aplicarlo al flujo de generación de estimación final, no a la extracción deproject_metadatani a respuestas auxiliares.
...
**Hay un techo de calidad que no se rompe refinando prompts.**Cuando el problema es de verificación, no de instrucción, la solución es estructural: separar generación de evaluación.

1. 

**Tres roles, no dos.**Actor genera, Critic evalúa, Boss decide. La separación entre evaluación y decisión rompe los modos de fallo clásicos del Self-Refine puro: bucles infinitos por insatisfacción crónica y sesgo de confirmación temprana.

1. 

**El patrón está sólidamente fundamentado en la literatura.**Actor-Critic-Boss compone evaluator-optimizer y orchestrator-workers, los dos workflows formalizados por Anthropic en*Building Effective Agents*, sin frameworks adicionales.

1. 

**El patrón compensa cuando el coste del error es alto, los criterios de evaluación son claros y la latencia extra es tolerable.**Aplícalo a los caminos críticos del producto, no a todas las llamadas al LLM.
