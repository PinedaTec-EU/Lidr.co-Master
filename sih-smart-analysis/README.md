...
By default the service reads reports from the TravelAgent bootstrap SIH output folder:
...
Reports are selected from the catalog instead of pasted manually. CAG mode analyzes the latest `N` reports for the selected workflow/environment. Semantic mode uses the selected catalog report as the current execution and retrieves similar historical reports from the repository. The sidebar has a separate `Reports para LLM` control, defaulting to `3`, that caps how many reports are sent to the LLM prompt while leaving the deterministic analysis window unchanged.

The report directory can be changed from the sidebar. The selector offers known local report folders, a native system folder picker when the local Python runtime can open one, and a manual path input for validating reports generated by other repositories without changing `.env`.

The LLM model can also be changed from the sidebar for local comparison runs. Presets cover OpenAI, Anthropic, NVIDIA NIM, and local Ollama models, and the manual field accepts any LiteLLM model id supported by the installed provider configuration.

When LLM analysis is enabled, the Streamlit wrapper streams `llm_insights` into the chat while LiteLLM returns deltas. During generation, the streamed answer ends with a live status line showing input tokens, output tokens, total tokens, output tokens per second, and elapsed time. The sidebar updates with the same metrics and keeps the final values for the last LLM analysis.

LLM analysis receives the deterministic score plus compacted report evidence from run-level and stage-level fields such as `Output`, `WorkflowOutput`, `WorkflowResult`, request/response bodies, preflight data, token metrics, model names, and provider outputs. The prompt also includes a quantitative timing summary by stage or endpoint with average, min, max, latest duration, and latest-vs-average deviation so the model can produce deeper latency and regression analysis. This keeps the UI useful when validating reports from other repositories with richer or slightly different SIH output shapes.
